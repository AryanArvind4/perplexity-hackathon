...
//             <main className="pt-[72px] font-sans text-black">
//                 {/* Hero Section */}
...
//                     <div className="grid grid-cols-1 md:grid-cols-2 gap-10 px-4 md:px-16 w-full">
//                         {[{
//                             video: 'feature1.mp4',
...
//                             <input type="text" placeholder="Budget" value={budget} onChange={(e) => setBudget(e.target.value)} className="w-full p-3 rounded-md border" />
//                             <input type="number" placeholder="Days" value={days} onChange={(e) => setDays(e.target.value)} className="w-full p-3 rounded-md border" />
...
//                 {/* Updated Itinerary Section with Beautiful Markdown */}
...
//                         {itinerary ? (
//                             <div className="bg-white rounded-xl shadow-lg p-8 md:p-12 text-left prose prose-lg prose-blue max-w-none mx-auto">
...
import axios from 'axios';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm'; // For table and markdown enhancements
                ...
                <section id="search" className="py-20 bg-gray-100 text-black w-full">
                    <div className="max-w-2xl mx-auto text-center">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <input type="text" placeholder="Movie" value={movie} onChange={(e) => setMovie(e.target.value)} required className="w-full p-3 rounded-md border" />
                            <input type="text" placeholder="Location" value={location} onChange={(e) => setLocation(e.target.value)} className="w-full p-3 rounded-md border" />
                            <input type="text" placeholder="Budget (Local Currency of the visiting country)" value={budget} onChange={(e) => setBudget(e.target.value)} className="w-full p-3 rounded-md border" />
                            <input type="number" placeholder="Days" value={days} onChange={(e) => setDays(e.target.value)} className="w-full p-3 rounded-md border" />
                            <input type="number" placeholder="Nights" value={nights} onChange={(e) => setNights(e.target.value)} className="w-full p-3 rounded-md border" />
                            <button type="submit" className="bg-gradient-to-r from-purple-700 to-pink-600 text-white px-6 py-3 rounded-full font-semibold shadow-md">
                                {loading ? 'Generating...' : 'Generate Itinerary'}
                    ...
                    <div className="max-w-4xl mx-auto text-center px-4">
                        ...
                        {itinerary ? (
                            ...
                            <ReactMarkdown
                                ...
                                // className="prose prose-lg prose-blue max-w-none mx-auto text-left bg-white rounded-lg p-6 shadow-lg"
                                components={{
                            ...
                            >
                                {/* Processed itinerary with custom cleanup (removing markdown table header if needed) */}
